		...
		try:
			if mac_payload.lower().startswith(startbytes+":"):
				...
				if session_started == "False" and str(mac_payload) != endmac:
					...
					octet = str(mac_payload).split(":")[1] # get the second octet for chunk num
					...
					chunk_num = str(mac_payload).split(":")[1] # get the second octet for chunk num
					chunk_num_dec = int(chunk_num, 16) # get the size
					byte1 = str(mac_payload).split(":")[2] # get the second octet for chunk num
					byte2 = str(mac_payload).split(":")[3] # get the second octet for chunk num
					byte3 = str(mac_payload).split(":")[4]
					byte4 = str(mac_payload).split(":")[5]
